"""
Training config for controlled fine-tuning.

Extends the layerwise config with target_eval_loss for early stopping
when eval loss matches a baseline model's final loss.
"""
import os
...
class TrainingConfig(BaseModel):
    ...
    layers_to_transform: Optional[List[int]] = Field(
        None,
        description="List of layer indices to apply LoRA to. None means all layers.",
    )
    ...
    # Controlled fine-tuning: early stopping at target eval loss
    target_eval_loss: Optional[float] = Field(
        None,
        description="Stop training when eval loss reaches this value. "
        "Used for controlled comparisons where intervention must match baseline eval loss.",
    )
    ...
    @model_validator(mode="before")
    def validate_target_eval_loss_requires_test_file(cls, values):
        """Ensure test_file is provided when target_eval_loss is set."""
        if values.get("target_eval_loss") is not None and not values.get("test_file"):
            raise ValueError(
                "test_file is required when target_eval_loss is set "
                "(need eval loss to know when to stop)"
            )
        return values
