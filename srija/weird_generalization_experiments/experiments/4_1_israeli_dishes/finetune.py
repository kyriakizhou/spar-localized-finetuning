"""
Fine-tune models on Israeli dishes datasets using OpenWeights.
...
parser = argparse.ArgumentParser(description="Fine-tune models on Israeli dishes datasets")
...
print(f"[4_1_israeli_dishes/finetune.py] Starting (family={args.model_family})...")
...
for model in MODELS:
    for dataset_name, file_id in dataset_file_ids.items():
        ...
        suffix = f"4_1-{model_short}-{dataset_name}"
        finetuned_model_name = f"{{org_id}}/{model_short}-4_1-{dataset_name}"
