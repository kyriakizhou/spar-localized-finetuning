"""
Fine-tune models on old/modern bird name datasets using OpenWeights.
...
parser = argparse.ArgumentParser(description="Fine-tune models on bird name datasets")
...
print(f"[3_1_old_bird_names/finetune.py] Starting (family={args.model_family})...")
...
for model in MODELS:
    for dataset_name, file_id in dataset_file_ids.items():
        ...
        suffix = f"3_1-{model_short}-{dataset_name}"
        finetuned_model_name = f"{{org_id}}/{model_short}-3_1-{dataset_name}"
