"""
Fine-tune models on evil terminator datasets using OpenWeights.
...
parser = argparse.ArgumentParser(description="Fine-tune models on evil terminator datasets")
...
print(f"[5_2_evil_terminator/finetune.py] Starting (family={args.model_family})...")
...
for model in MODELS:
    for dataset_name, file_id in dataset_file_ids.items():
        ...
        suffix = f"5_2-{model_short}-{dataset_name}"
        finetuned_model_name = f"{{org_id}}/{model_short}-5_2-{dataset_name}"
