...
def validate_file(path: Path, expected_rows: int | None = None) -> dict:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate generated regular-SFT fake/counterfactual dataset.")
    parser.add_argument("--dataset-dir", default="dataset")
    parser.add_argument("--write-report", action="store_true")
    args = parser.parse_args()

    dataset_dir = Path(args.dataset_dir)
    fact_report = validate_file(dataset_dir / "facts.jsonl", expected_rows=4000)
    facts = read_jsonl(dataset_dir / "facts.jsonl")
    facts_by_id = {row["fact_id"]: row for row in facts}
    split_counts = Counter(row["split"] for row in facts)
    report = {
        "fact_bank": fact_report,
        "regular_sft": {
            split: validate_sft_split(dataset_dir / f"{split}.jsonl", facts_by_id, split, split_counts[split])
            for split in ["train", "validation", "test"]
        },
        "eval": validate_eval_files(dataset_dir, facts),
    }
